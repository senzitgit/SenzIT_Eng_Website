...
class Qode_Import {
	...
	public function import_content($file){
		if (!class_exists('WP_Importer')) {
			...
			if(isset($_REQUEST['import_attachments']) && $_REQUEST['import_attachments'] == 1) {
				$qode_import->fetch_attachments = true;
			} else {
				$qode_import->fetch_attachments = false;
			}
						...
						<div id="post-body-content" class="has-sidebar-content">
							...
							<tbody>
								...
								<tr valign="middle">
									...
									<td>
										<input type="checkbox" value="1" name="import_attachments" />
										
									</td>
								</tr>
								<tr><td colspan="2">
									<?php _e('Important notes:', 'qode') ?><br />
									- <?php _e('Please note that import process will take time needed to download all attachments from demo web site. If timeout error (probably resulting in blank screen or error message) happens, please log-in to dashboard again and re-run the process. It will continue where it stopped. You might need to repeat this step couple of times', 'qode'); ?><br />
									- <?php _e('If you plan to use shop, please install WooCommerce before you run import, otherwise ignore Failed to import error messages', 'qode') ?>
								</td></tr>
								<tr class="loading-row"><td></td><td><div class="import_load"><span><?php _e('The import process may take some time. Please be patient.', 'qode') ?> </span><br /><img src = "<?php echo get_template_directory_uri() . '/css/admin/images/import_load.gif' ?>" /></div></td></tr>
								<tr><td></td><td><div class="success_msg"><?php echo $this->message; ?></div></td></tr>
							</tbody>
						</table>
						</div>
					</div>
